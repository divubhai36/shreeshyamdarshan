          ...
          <div className="lg:col-span-1 space-y-6">
            <div className="bg-white p-8 lg:p-10 rounded-[40px] border border-brand-primary/5 shadow-xl relative overflow-hidden">
               ...
               <div className="mt-12 space-y-2">
                  <LinkTab icon="solar:history-bold" label="Order History" active />
                               ...
                               {order.items.map((item, idx) => (
                                 <div key={idx} className="flex gap-4 items-center bg-brand-accent/30 p-4 rounded-2xl border border-brand-primary/5">
                                    ...
                                    <div className="text-left">
                                       ...
                                       <p className="text-[9px] font-bold text-brand-primary/40 mt-1 uppercase tracking-widest">{item.quantity} units @ ₹{item.price.toLocaleString()}</p>
                                    </div>
                                 </div>
                               ))}
                            </div>
...
function LinkTab({ icon, label, active = false, path = "#" }) {
  const router = useRouter();
  return (
    <button 
      onClick={() => path !== "#" && router.push(path)}
      className={`w-full flex items-center gap-4 p-4 rounded-2xl transition-all font-bold text-[10px] uppercase tracking-widest ${active ? 'bg-brand-primary text-white shadow-xl' : 'text-brand-primary/40 hover:bg-brand-primary/5 hover:text-brand-primary'}`}
    >
