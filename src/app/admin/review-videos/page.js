...
import { getReviewVideos, createReviewVideo, deleteReviewVideo } from "../actions";
...
export default function ReviewVideosPage() {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    if (!form.url && !selectedFile) return toast.error("Please provide a video URL");

    let currentForm = { ...form };

    try {
      if (selectedFile) {
        toast.loading("Uploading Testimonial...", { id: 'rev' });
        const res = await uploadToAllAccounts(selectedFile, 'video');
        currentForm.url = res.public_id;
      }

      toast.loading("Recording Review...", { id: 'rev' });
      await createReviewVideo(currentForm);
      toast.success("Review video added", { id: 'rev' });
      setIsOpen(false);
      setForm({ title: "", url: "" });
      ...
      toast.error("Failed to add video. Sync error.", { id: 'rev' });
    }
  };

  const handleDelete = async (id) => {
    if (confirm("Remove this review video?")) {
      try {
  ...
  return (
    <div className="max-w-6xl mx-auto">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-end mb-8 gap-6">
        <div>
          <h1 className="text-4xl font-serif font-bold text-brand-primary">Customer Review Videos</h1>
          <p className="text-[10px] font-black text-brand-secondary tracking-[0.4em] uppercase mt-2 opacity-60">Testimonial Reels Management</p>
        </div>
        <button
          onClick={() => {
            setForm({ title: "", url: "" });
        ...
        >
          <Icon icon="lucide:plus" className="w-4 h-4" /> Add Review Video
        </button>
      </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-5 gap-6">
          {data.map((video) => (
            <div key={video.id} className="bg-white rounded-3xl shadow-sm border border-brand-primary/5 overflow-hidden group relative aspect-[9/16]">
              ...
              <div className="absolute inset-0 bg-linear-to-t from-brand-primary/80 to-transparent opacity-0 group-hover:opacity-100 transition-all flex flex-col justify-end p-4">
                <p className="text-white text-[10px] font-bold uppercase tracking-wider mb-2 truncate">{video.title || "Customer Review"}</p>
          ...
          {data.length === 0 && (
            <div className="col-span-full py-20 text-center bg-brand-primary/5 rounded-[40px] border-2 border-dashed border-brand-primary/10">
              ...
              <p className="text-brand-primary/40 font-serif italic">No customer review videos added yet.</p>
            </div>
          )}
        </div>
      )}
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-brand-primary/50 backdrop-blur-md p-4">
          <div className="bg-white max-w-md w-full rounded-3xl shadow-2xl overflow-hidden flex flex-col">
            <div className="flex justify-between items-center px-6 py-4 border-b border-brand-primary/5">
              <div>
                <h2 className="text-xl font-bold text-brand-primary font-serif">Add Review Reel</h2>
                <p className="text-[9px] text-brand-secondary font-bold uppercase tracking-widest">Customer Testimonial</p>
              </div>
              <button
                onClick={() => {
                  setIsOpen(false);
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-6">
              <div>
                <label className="text-[9px] uppercase font-black tracking-[0.15em] text-brand-primary block mb-1.5 leading-none">Customer Name/Title (Optional)</label>
                <input
                  type="text"
                  value={form.title}
                  onChange={(e) => setForm({ ...form, title: e.target.value })}
                  placeholder="e.g. Pooja from Mumbai"
