    ...
    <div className="max-w-7xl mx-auto">
      <div className="flex justify-between items-center mb-8">
        <div>
          ...
          <p className="text-xs font-bold text-brand-secondary tracking-widest uppercase">Administer inventory</p>
        </div>
        <button onClick={() => { setEditingId(null); setForm(initForm); setIsOpen(true); }} className="bg-brand-primary text-white px-8 py-3 rounded-2xl font-bold text-[10px] uppercase tracking-widest hover:shadow-2xl transition-all shadow-xl">
          + Inject Product
        </button>
      </div>
            ...
            <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-2 gap-8">
                ...
                <div className="space-y-5">
                    <div>
                        ...
                        <select value={form.categoryId} onChange={e=>setForm({...form, categoryId: e.target.value, subCategoryId: '', innerSubId: null})} className="w-full p-4 border border-black/10 rounded-2xl bg-gray-50 mb-2" required>
                            <option value="">1. Select Category</option>
                            {cats.map(c => <option key={c.id} value={c.id}>{c.name}</option>)}
                        </select>
                        <select value={form.subCategoryId} onChange={e=>setForm({...form, subCategoryId: e.target.value, innerSubId: null})} className="w-full p-4 border border-black/10 rounded-2xl bg-gray-50 mb-2" required disabled={!form.categoryId}>
                            <option value="">2. Select Sub-category</option>
                            {availableSubs.map(c => <option key={c.id} value={c.id}>{c.name}</option>)}
                        </select>
                        <select value={form.innerSubId || ""} onChange={e=>setForm({...form, innerSubId: e.target.value})} className="w-full p-4 border border-black/10 rounded-2xl bg-gray-50" disabled={!form.subCategoryId || availableInners.length===0}>
                            <option value="">3. Optional Inner Filter</option>
                            {availableInners.map(c => <option key={c.id} value={c.id}>{c.name}</option>)}
                        </select>
                    </div>
