...
const getCollectionData = cache(async (categoryId) => {
  try {
    ...
    console.error("[Collections] getCollectionData error:", err?.message);
    return null;
  }
});
...
export async function generateStaticParams() {
  try {
    ...
    console.error("[Collections] generateStaticParams error:", err?.message);
    return [];
  }
}
...
export async function generateMetadata({ params }) {
  ...
  const dbCat = await getCollectionData(categoryId);
  if (dbCat) {
    return {
...
export default async function CollectionsPage({ params }) {
  ...
  const dbCategory = await getCollectionData(categoryId);

  if (dbCategory) {
     const subCategoriesData = dbCategory.subCategories.map((sub) => ({
        id: sub.slug,
        name: sub.name,
